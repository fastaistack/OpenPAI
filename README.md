...
## 📦 安装与部署
以下**安装部署步骤**皆为在服务器上进行的**命令行**操作，小白用户请谨慎操作。

整体安装部署思路是：**环境准备 → 拉镜像 → 准备模型 → 部署（部署数据库 → 部署 Chat → 部署 LLM-Serving)**。

### 一、环境准备：

本项目主要采用K8s部署的方式，后续会完善docker部署方式。请准备至少 1 台机器（支持多节点更佳）。

硬件要求：

- 推荐配置： CPU：50核  + 内存：90GB + GPU(Nvidia)：1*(显存60G以上) + 磁盘：170GB 
...
chat部署的yaml文件在项目的**deploymnet/k8s/chat**目录下，先将其单独拷贝出来放到**主节点**任意目录下。
...
根据实际情况修改**openpai_config.yml**文件中的值，其中**{{HOST_IP}}**和**{{HOST_PATH}}**可忽略。
...
**如下脚本主要用于替换all-openpai-deployment.yml中的值，可以根据实际情况手动指定all-openpai-deployment.yml中的部署方式（4个文件夹要建好并给与权限）。**
...
LLM-Serving部署的yaml文件在项目的**deploymnet/k8s/LLM-Serving**目录下，先将项目克隆其单独拷贝出来放到**主节点**任意目录下。
...
根据实际情况修改**llm-serving_config.yml**文件中的值：其中**{{HOST_IP}}**和**{{HOST_PATH}}**可忽略。
...
**如下脚本主要用于替换llm-serving.yml中的值，可以根据实际情况手动指定llm-serving.yml中的部署方式。**
